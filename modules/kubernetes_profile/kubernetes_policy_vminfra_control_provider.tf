...
resource "intersight_kubernetes_virtual_machine_infrastructure_provider" "vminfra-control-provider-policy" {
  name        = "${var.cluster_name}-vminfra-control-node-profile"
  description = "terraform - Kubernetes VM Infra Provider Policy"

  node_group {
    moid = intersight_kubernetes_node_group_profile.control-node-profile.moid
  }
